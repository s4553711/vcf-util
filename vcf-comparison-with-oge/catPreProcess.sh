...
grep '^#' .venn/${name}.$(head -n 1 $chrSource).decomplx.brk.decomplx.vcf > .venn/data/${name}.final.vcf
for chrosome in $chrs; do
	cat .venn/${name}.${chrosome}.decomplx.brk.decomplx.vcf | grep -v '#' >> .venn/data/${name}.final.vcf
done
